...
describe('encodeFilterData', function () {
    ...
    it('Returns an error when passed an arg for an un-indexed parameter', function () {
        var filter = {
            contract: token,
            event: 'Transfer',
            args: [constants_1.AddressZero, constants_1.AddressZero, 10]
        };
        var encodedFilterData = (0, logs_1.encodeFilterData)(filter, 0, 'latest');
        (0, chai_1.expect)(encodedFilterData).to.be.a('Error');
    });
    it('Returns an error when passed too many args', function () {
        var filter = {
            contract: token,
            event: 'Transfer',
            args: [constants_1.AddressZero, constants_1.AddressZero, null, constants_1.AddressZero]
        };
        var encodedFilterData = (0, logs_1.encodeFilterData)(filter, 0, 'latest');
        (0, chai_1.expect)(encodedFilterData).to.be.a('Error');
    });
...
describe('decodeLogs', function () {
        ...
        var logs = [
            {
                ...
                topics: [
                    ethers_1.ethers.utils.id('Transfer2(address,address,uint256)'),
                    ethers_1.ethers.utils.hexZeroPad(constants_1.AddressZero, 32),
                    ethers_1.ethers.utils.hexZeroPad(constants_1.AddressZero, 32),
                ],
                data: ethers_1.ethers.utils.hexZeroPad(constants_1.AddressZero, 32),
                blockHash: '0x0',
                blockNumber: 0,
                logIndex: 0,
                transactionIndex: 0,
    ...
    it('Works when passed valid logs', function () {
        ...
        var from = constants_1.AddressZero;
        var to = deployer.address;
        var value = ethers_1.BigNumber.from(1);
        var blockHash = '0x0';
        var blockNumber = 1;
        var logIndex = 2;
        ...
        var logs = [
            {
                address: token.address,
                topics: [
                    ethers_1.ethers.utils.id('Transfer(address,address,uint256)'),
                    ethers_1.ethers.utils.hexZeroPad(from, 32),
                    ethers_1.ethers.utils.hexZeroPad(to, 32),
                ],
                data: ethers_1.ethers.utils.hexZeroPad(ethers_1.ethers.utils.hexlify(value), 32),
                blockHash: blockHash,
